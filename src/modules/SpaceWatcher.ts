...
export class SpaceWatcher extends EventEmitter {
  ...
  private async downloadAudio() {
    ...
    try {
      ...
      const filePath = path.join(Util.getMediaDir(this.space?.creator?.username), `${this.filename}.m4a`)
      const playlistPath = path.join(Util.getMediaDir(this.space?.creator?.username), `${this.filename}.m3u8`)
  ...
  private sendWebhooksWithS3Url(s3AudioUrl: string, s3PlaylistUrl?: string) {
    ...
    const webhook = new Webhook(this.space, this.audioSpace, combinedS3Url)
    webhook.send()
  }
